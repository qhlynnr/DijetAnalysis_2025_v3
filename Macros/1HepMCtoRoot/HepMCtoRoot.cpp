...
int main(int argc, char** argv) {
    ...
    std::vector<double>* px_v = new std::vector<double>();
    std::vector<double>* py_v = new std::vector<double>();
    std::vector<double>* pz_v = new std::vector<double>();
    std::vector<double>* e_v = new std::vector<double>();
    std::vector<int>* pdgId_v = new std::vector<int>();
    std::vector<double>* pt_v = new std::vector<double>();
    std::vector<double>* p_v = new std::vector<double>();
    std::vector<double>* eta_v = new std::vector<double>();
    std::vector<double>* phi_v = new std::vector<double>();
    ...
    particleTree->Branch("weights", &weights);
    ...
    while ((evt = input.read_next_event())) {
        ...
        for (auto p = evt->particles_begin(); p != evt->particles_end(); ++p) {
            ...
            double pt = sqrt(mom.px()*mom.px() + mom.py()*mom.py());
            double part_p = sqrt(mom.px()*mom.px() + mom.py()*mom.py() + mom.pz()*mom.pz());
            double eta = -999;
            ...
            double phi = atan2(mom.py(), mom.px());
        ...
        if (ievt < 10) {
            cout << "Event " << ievt << ": nVertices=" << nVertices
