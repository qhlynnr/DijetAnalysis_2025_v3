...
void plotHistograms(vector<TH1F*> histograms,
    ...
    // append suffix for log if requested
    string suffix = logy ? "_log" : "";
...
int DrawJet(){
    ...
    TH1F *h_ratio_raw = (TH1F*)h_jetpt_raw->Clone("h_ratio_raw");
    h_ratio_raw->Divide(h_jetpt_vac);

    TH1F *h_ratio = (TH1F*)h_jetpt->Clone("h_ratio");
    h_ratio->Divide(h_jetpt_vac);

    h_ratio_raw->GetXaxis()->SetRangeUser(50,1000);
    h_ratio->GetXaxis()->SetRangeUser(50,1000);
    ...
    plotHistograms({h_jetpt, h_jetpt_raw, h_jetpt_thermal,h_jetpt_vac}, 
                    ...
                    true);
    plotHistograms({h_ratio, h_ratio_raw}, 
                    {"R_{AA}", " R_{AA} Raw Only"}, 
